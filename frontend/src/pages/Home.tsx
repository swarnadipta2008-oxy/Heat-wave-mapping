    ...
    <div className="min-h-screen overflow-x-hidden bg-[#0A0E17] text-slate-100">
      <header className="sticky top-0 z-50 border-b border-white/[.07] bg-[#0A0E17]/85 backdrop-blur-xl" data-testid="main-navbar">
        <div className="mx-auto flex max-w-[1480px] items-center justify-between px-5 py-3 lg:px-8">
          <a href="#overview" className="flex items-center gap-3" data-testid="brand-home-link">
            <span className="grid h-9 w-9 place-items-center rounded-xl bg-gradient-to-br from-ember-400 to-sindoor-600 shadow-lg shadow-orange-950/30"><ThermometerSun className="h-5 w-5 text-white" /></span>
            <span><span className="block font-heading text-sm font-bold tracking-tight text-white">HeatMap <span className="text-ember-400">India</span></span><span className="block font-mono text-[9px] uppercase tracking-[.18em] text-slate-500">EVS · CA1 PROJECT</span></span>
          </a>
      ...
      <main>
        ...
        <section id="map" className="section-shell scroll-mt-20" data-testid="interactive-map-section">
          ...
          <div className="mb-5 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between"><div className="relative max-w-sm flex-1"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-500" /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search a city..." className="h-10 w-full rounded-lg border border-white/10 bg-white/[.04] pl-10 pr-3 text-sm text-white outline-none placeholder:text-slate-600 focus:border-ember-400/50" data-testid="city-search-input" />{search && <div className="absolute left-0 right-0 top-12 z-20 rounded-xl border border-white/10 bg-[#151d2c] p-1 shadow-2xl">{filteredCities.slice(0, 5).map((city) => <button key={city.id} onClick={() => { selectCity(city.id); setSearch(city.name); }} className="flex w-full items-center justify-between rounded-lg px-3 py-2 text-left text-xs hover:bg-white/[.07]" data-testid={`search-result-${city.id}`}><span className="text-slate-200">{city.name}</span><span className="font-mono text-slate-500">{city.score}</span></button>)}</div>}</div><div className="flex flex-wrap gap-1.5" data-testid="risk-filter-group">{(["all", "very_high", "high", "moderate", "low"] as const).map((tier) => <button key={tier} onClick={() => setFilter(tier)} className={`rounded-full border px-3 py-2 text-[10px] font-semibold  transition-colors ${filter === tier ? "border-ember-400/50 bg-ember-400/15 text-ember-300" : "border-white/10 text-slate-500 hover:border-white/20 hover:text-slate-300"}`} data-testid={`risk-filter-${tier}`}>{tier === "all" ? "All cities" : tierMeta[tier].label}</button>)}</div></div>
          <div className="mb-5 flex flex-wrap items-center gap-3 rounded-xl border border-white/[.07] bg-white/[.025] p-3" data-testid="map-toolkit"><div className="flex items-center gap-2 text-[10px]  text-slate-500"><Layers3 className="h-3.5 w-3.5 text-ember-300" /> Map layers</div><select value={mapLayer} onChange={(event) => setMapLayer(event.target.value as MapLayer)} className="h-8 rounded-lg border border-white/10 bg-[#111827] px-2 text-xs text-slate-200 outline-none focus:border-ember-400/50" data-testid="map-layer-selector" aria-label="Map layer"><option value="cartographic">Cartographic vector</option><option value="thermal">Thermal overlay</option><option value="grid">Tactical grid</option></select><span className="h-5 w-px bg-white/10" /><label className="text-[10px]  text-slate-500" htmlFor="map-region-filter">Region focus</label><select id="map-region-filter" value={activeRegion} onChange={(event) => setActiveRegion(event.target.value)} className="h-8 rounded-lg border border-white/10 bg-[#111827] px-2 text-xs text-slate-200 outline-none focus:border-ember-400/50" data-testid="map-region-filter"><option value="all">All India</option>{mapRegions.map((region) => <option key={region.id} value={region.id}>{region.label}</option>)}</select>{activeRegion !== "all" && <button type="button" onClick={() => setActiveRegion("all")} className="rounded-lg px-2 py-1.5 text-[10px] text-ember-300 hover:bg-ember-400/10" data-testid="clear-map-region-button">Clear region</button>}<div className="flex items-center gap-1 rounded-lg border border-white/[.08] bg-[#111827] p-1" data-testid="map-navigation-controls"><button type="button" onClick={() => zoomMap(.2)} className="map-control-button" data-testid="map-zoom-in" aria-label="Zoom in"><ZoomIn className="h-4 w-4" /></button><button type="button" onClick={() => zoomMap(-.2)} className="map-control-button" data-testid="map-zoom-out" aria-label="Zoom out"><ZoomOut className="h-4 w-4" /></button><button type="button" onClick={resetMap} className="map-control-button" data-testid="map-reset-view" aria-label="Reset map view"><RotateCcw className="h-4 w-4" /></button></div><span className="ml-auto hidden text-[10px] text-slate-500 sm:inline">Click a shaded region or marker · drag to pan</span></div>
          <div className="grid gap-5 xl:grid-cols-[1.65fr_1fr]">
            <div className="map-shell relative min-h-[520px] overflow-hidden rounded-2xl border border-white/[.08] bg-[#0c1421]" data-testid="india-map-canvas">
              ...
              <div className="absolute left-5 top-5 z-20 flex items-center gap-2"><span className="rounded-md border border-white/10 bg-[#111827]/85 px-2.5 py-1.5 font-mono text-[9px]  text-slate-400">INDIA / VECTOR ATLAS</span><span className="hidden rounded-md border border-monsoon-400/20 bg-monsoon-400/[.08] px-2.5 py-1.5 font-mono text-[9px]  text-monsoon-300 sm:inline">{mapView.scale.toFixed(1)}× VIEW</span></div>
              <div className="absolute right-5 top-5 z-20 hidden rounded-md border border-white/10 bg-[#111827]/85 px-2.5 py-1.5 font-mono text-[9px]  text-slate-500 sm:block">CALIBRATED LAT / LNG · DEMO</div>
        ...
        <section id="compare" className="section-shell scroll-mt-20" data-testid="city-comparison-section">
          ...
          <div className="mb-5 flex gap-2 overflow-x-auto pb-2" data-testid="comparison-city-selector">{cities.map((city) => <button key={city.id} onClick={() => toggleCompare(city.id)} className={`flex shrink-0 items-center gap-2 rounded-xl border px-3 py-2 text-left transition-all ${compareIds.includes(city.id) ? "border-ember-400/40 bg-ember-400/[.1]" : "border-white/[.08] bg-white/[.025] opacity-60 hover:opacity-100"}`} data-testid={`compare-toggle-${city.id}`}><span className="h-2 w-2 rounded-full" style={{ background: tierMeta[city.tier].color }} /><span className="text-[11px] text-slate-200">{city.name}</span>{compareIds.includes(city.id) && <span className="text-[10px] text-ember-300">✓</span>}</button>)}</div>
        ...
        <section id="calculator" className="section-shell scroll-mt-20" data-testid="risk-calculator-section">
          ...
          <div className="grid gap-5 lg:grid-cols-[1fr_.8fr]"><div className="dashboard-card rounded-2xl p-5 sm:p-7" data-testid="risk-calculator-form"><div className="mb-6 flex items-center justify-between"><div><p className="eyebrow">Scenario simulator</p><h3 className="mt-1 font-heading text-lg font-semibold text-white">Build a city scenario</h3></div><Calculator className="h-5 w-5 text-ember-300" /></div><div className="grid gap-x-8 gap-y-6 sm:grid-cols-2">{([ ["max_temp", "Maximum temperature", `${calcValues.max_temp}°C`, 30, 48, 0.1], ["heatwave_days", "Heatwave days / year", `${calcValues.heatwave_days} days`, 0, 30, 1], ["pop_exposure", "Population exposure", `${calcValues.pop_exposure}%`, 0, 100, 1], ["urbanization", "Urbanization", `${calcValues.urbanization}%`, 0, 100, 1], ["vegetation_cover", "Green cover buffer", `${calcValues.vegetation_cover}%`, 0, 100, 1]] as const).map(([key, label, display, min, max, step]) => <label key={key} className="space-y-2" data-testid={`calculator-field-${key}`}><span className="flex items-center justify-between text-xs text-slate-400"><span>{label}</span><span className="font-mono text-ember-300">{display}</span></span><input type="range" min={min} max={max} step={step} value={calcValues[key]} onChange={(event) => setCalcValues((current) => ({ ...current, [key]: Number(event.target.value) }))} className="risk-slider w-full" data-testid={`calculator-slider-${key}`} /></label>)}</div><div className="mt-7 rounded-xl border border-white/[.07] bg-black/10 p-4"><p className="eyebrow">Model equation</p><p className="mt-2 text-xs leading-6 text-slate-400">Temperature <b className="text-ember-300">30%</b> + Frequency <b className="text-ember-300">25%</b> + Population <b className="text-ember-300">20%</b> + Urbanization <b className="text-ember-300">15%</b> + (100 − Vegetation) <b className="text-monsoon-300">10%</b></p></div><Button onClick={() => calculateMutation.mutate(calcValues)} disabled={calculateMutation.isPending} className="mt-6 h-10 bg-ember-400 text-white hover:bg-ember-400" data-testid="calculate-risk-button">{calculateMutation.isPending ? "Calculating…" : "Calculate scenario score"}<ArrowUpRight className="ml-2 h-4 w-4" /></Button></div><div className="thermal-panel rounded-2xl p-6 sm:p-7" data-testid="calculator-result-panel"><p className="eyebrow text-ember-300">Live scenario preview</p><div className="mt-8 flex items-end justify-between"><div><p className="font-mono text-6xl font-bold text-white" data-testid="calculator-preview-score">{calculateMutation.data?.score ?? previewScore}</p><p className="mt-1 text-xs  text-slate-500">out of 100</p></div><RiskBadge tier={calculateMutation.data?.tier ?? (previewScore >= 75 ? "very_high" : previewScore >= 60 ? "high" : previewScore >= 40 ? "moderate" : "low")} /></div><div className="mt-8 space-y-3"><div className="flex justify-between text-xs"><span className="text-slate-400">Hazard signal</span><span className="font-mono text-slate-200">{Math.round(((calcValues.max_temp - 30) / 18 * 100) * .3 + Math.min(100, calcValues.heatwave_days / 24 * 100) * .25)} pts</span></div><div className="h-2 rounded-full bg-white/[.08]"><div className="h-full rounded-full bg-gradient-to-r from-monsoon-400 via-ember-400 to-sindoor-500" style={{ width: `${previewScore}%` }} /></div><p className="text-xs leading-5 text-slate-400">Higher green cover reduces the score because vegetation can provide shade and evaporative cooling.</p></div><div className="mt-8 border-t border-white/[.08] pt-4 text-[10px]  text-slate-500">Educational simulation · not an official warning</div></div></div>
        </section>
        ...
        <section id="methodology" className="section-shell scroll-mt-20" data-testid="methodology-section">
          ...
          <div className="grid gap-5 lg:grid-cols-[1fr_1fr]"><div className="dashboard-card rounded-2xl p-6 sm:p-7" data-testid="methodology-model-card"><div className="flex items-start gap-4"><span className="number-chip">01</span><div><h3 className="font-heading text-lg font-semibold text-white">Hazard × exposure × context</h3><p className="mt-2 text-sm leading-6 text-slate-400">The demonstration combines a temperature signal and heatwave frequency with population exposure, urbanization pressure, and a vegetation cooling buffer. Each input is normalized to a 0–100 index.</p></div></div><div className="my-7 flex flex-wrap items-center gap-2 text-xs"><span className="formula-chip">Temperature <b>30%</b></span><span className="text-slate-600">+</span><span className="formula-chip">Frequency <b>25%</b></span><span className="text-slate-600">+</span><span className="formula-chip">Exposure <b>20%</b></span><span className="text-slate-600">+</span><span className="formula-chip">Urban <b>15%</b></span><span className="text-slate-600">+</span><span className="formula-chip">Green buffer <b>10%</b></span></div><div className="flex gap-3 border-t border-white/[.07] pt-5 text-xs leading-5 text-slate-500"><CircleHelp className="h-4 w-4 shrink-0 text-ember-300" /> The weights are a simple educational assumption for demonstrating multi-factor assessment. They should be reviewed before any research or policy use.</div></div><div className="dashboard-card rounded-2xl p-6 sm:p-7" data-testid="about-project-card"><div className="flex items-start gap-4"><span className="number-chip bg-sky-400/10 text-sky-300">02</span><div><h3 className="font-heading text-lg font-semibold text-white">Why cities need a heat lens</h3><p className="mt-2 text-sm leading-6 text-slate-400">Dense built surfaces can retain heat, while high population exposure can increase health impacts. Digital maps help students communicate where multiple pressures overlap and where cooling, shade, and preparedness deserve attention.</p></div></div><div className="mt-7 grid grid-cols-3 gap-2"><div className="rounded-xl bg-white/[.04] p-3 text-center"><Trees className="mx-auto h-5 w-5 text-monsoon-300" /><p className="mt-2 text-[10px] text-slate-400">Green cover</p></div><div className="rounded-xl bg-white/[.04] p-3 text-center"><Users className="mx-auto h-5 w-5 text-ember-300" /><p className="mt-2 text-[10px] text-slate-400">Exposure</p></div><div className="rounded-xl bg-white/[.04] p-3 text-center"><BarChart3 className="mx-auto h-5 w-5 text-sky-300" /><p className="mt-2 text-[10px] text-slate-400">Evidence</p></div></div><div className="mt-5 rounded-xl border border-sindoor-400/15 bg-sindoor-400/[.05] p-3 text-xs leading-5 text-slate-400"><b className="text-sindoor-200">Limitation:</b> This product is not an official IMD early-warning or government decision-support system.</div></div></div>
        </section>
